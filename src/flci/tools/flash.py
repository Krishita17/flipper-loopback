...
``device_info`` again.
    ...
    flci flash dist/f7-C/f7-update-local --port "$FLCI_DUT" [--expect-commit abc1234]
    flci flash flipper-z-f7-update-1.4.0.tgz --port ...
...
def main(argv: list[str] | None = None) -> None:
    p = argparse.ArgumentParser(
        prog=f"flci {'flash'}", description=__doc__, formatter_class=argparse.RawTextHelpFormatter
    )
