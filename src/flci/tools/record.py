...
Two modes:
  ...
  From a saved file (offline, deterministic):
    flci record from-sub  path/to/remote.sub  --id garage_433
    flci record from-ir   path/to/tv.ir --signal Power --id tv_power

  Live capture on one Flipper (whatever it decodes first becomes the expectation):
    flci record live subghz  --port /dev/cu.usbmodemflip_X1 --frequency 433920000 --id x
    flci record live infrared --port ... --id x
    flci record live ibutton  --port ... --id x
    flci record live rfid     --port ... --id x
...
def main(argv: list[str] | None = None) -> None:
    p = argparse.ArgumentParser(
        prog=f"flci {'record'}", description=__doc__, formatter_class=argparse.RawTextHelpFormatter
    )
